...
- `/app/board` - Team task board (Kanban **and** a sortable list view)
- `/app/projects` - Projects (group tasks under an engagement and track progress)
...
## 3. Core Features
- [x] Team authentication (email + password sign up then instant sign-in — no email-confirmation step; the account is stored under the person's name + email)
- [x] Team directory with profiles
- [x] Client directory
- [x] Task board with owners, priority, status, due date (create / edit / move / delete), viewable as a Kanban board or a sortable list (due date / priority / status / owner / recently updated / client)
- [x] Projects layer: group related tasks under a project (linked to a client), with rolled-up progress, open/done/overdue counts and a Projects page
...
- [x] Manager request buckets: Waiting for us / Waiting for the client / Going quiet / Unassigned / Overdue. A request "Waiting on Client" is never counted as overdue or stale.
- [x] Aging indicator on paused requests: shows how many days a request has been waiting on the client (fresh → ageing → going quiet) and auto-flags the ones quiet for 7+ days.
...
| client_id | uuid | FK -> clients (nullable; internal projects allowed) |
...
| project_id | uuid | FK -> projects (nullable; groups the task) |
...
| waiting_since | timestamptz | When the request last entered "waiting on client" (null when it isn't paused). Drives the aging indicator. |
| created_at | timestamptz | Created timestamp |
...
### Phase 10: Waiting-on-Client Aging Indicator (shipped)
- Goal: Let managers see how long a request has been stuck waiting on the client, and flag the ones that have gone quiet for too long, instead of those requests silently ageing in the background.
- Deliverable: `requests.waiting_since` timestamp (stamped when a request enters "waiting on client", cleared when it leaves) + `waitingDays` / `waitingAgeLevel` / `isClientWaitingStale` helpers (fresh < 3 days, ageing 3–6, going quiet 7+). The Request Card shows an escalating "Waiting Nd" chip, a red "Flagged — going quiet" note for 7+ day silences, and a new **Going quiet** manager bucket mirrored on the Overview pipeline card. [x] Done.

### Phase 11: Projects Layer + Sortable List View (shipped)
- Goal: Close the last gaps against the task-management brief — group work into projects, and give the board a second, sortable view.
- Deliverable: a `projects` table (name, client, status, accent colour) with `tasks.project_id`, a Projects page with rolled-up progress / open / done / overdue counts and project CRUD, a Project field on the task modal, a project filter on the board, and a **Board / List** view toggle where the list sorts by due date, priority, status, owner, recently updated or client (with asc/desc). [x] Done.

### Phase 12: Instant Sign-Up (no email confirmation) (shipped)
- Goal: Remove the Gmail confirmation detour so a new teammate can create an account with their name, email and password and land straight in the workspace.
- Deliverable: Signup requires the person's name (saved to `profiles.full_name` via the `handle_new_user` trigger). Account creation goes through the `create-account` Edge Function, which uses the Admin API with `email_confirm: true` so Supabase never sends a confirmation email at all — this removes the Gmail step, the "email rate limit exceeded" error, and any need to change Authentication settings. The client then signs in with the same email + password to get a real session, so every workspace table's row-level security keeps working. If the email already has an account, the form falls back to signing in (and tells the person to use their original password when it does not match). [x] Done.